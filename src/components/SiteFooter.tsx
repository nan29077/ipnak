...
/**
 * SiteFooter — 페이지별 관리자 스위치로 노출 여부를 제어할 수 있는 범용 푸터.
 * pageKey: 'home' | 'feed' | 'me' | 'map' | ... (관리자 설정의 키와 일치해야 함)
 */
...
export function SiteFooter({ pageKey }: Props) {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/footer")
      .then((r) => r.json())
      .then(({ visibility }: { visibility: Record<string, boolean> }) => {
        if (visibility?.[pageKey]) {
          setVisible(true);
          // 표시 대상일 때만 법적 데이터 로드
  ...
  try {
    company = JSON.parse(data.company_info) as CompanyInfo;
  } catch {
    // ignore
  }
