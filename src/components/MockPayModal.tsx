...
import { CreditCard, Landmark, Smartphone, ShieldCheck, X } from "lucide-react";
...
// PG 결제창 mock — 실제 PG(토스페이먼츠 등) 연동은 추후.
// 결제 연동 전까지는 실제 충전(포인트 발행)을 하지 않고 "준비 중" 안내만 표시한다.
...
export function MockPayModal({
  ...
  // TODO: 실제 PG 결제 승인 연동 지점 — 승인 성공 시에만 /api/points/charge 호출 후 onSuccess 전달
  function pay() {
    toast("현재 포인트 충전 서비스는 준비 중입니다.", "info");
  }
    ...
    <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/80 px-0 backdrop-blur-[3px] sm:items-center sm:px-5">
      ...
      >
        ...
        <div className="mx-5 mt-4 flex items-start gap-2 rounded-xl bg-white/[0.05] px-3 py-2.5 text-[11px] leading-relaxed text-white/55">
          ...
          <span><b className="text-white/80">포인트 충전 서비스는 준비 중</b>입니다. PG 결제 연동 전까지는 실제 청구·결제와 포인트 충전이 이루어지지 않습니다.</span>
        </div>
