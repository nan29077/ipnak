          ...
          {products.map((p) => {
            ...
            return (
              <button key={p.id} type="button" onClick={() => toggle(p.id)}
                className={cn("flex w-full items-center gap-3 rounded-xl border p-2 text-left transition-colors", on ? "border-orange-500/50 bg-orange-500/10" : "border-navy-100 hover:bg-navy-50/40")}>
                ...
                {on && <Check size={18} className="shrink-0 text-orange-400" />}
              </button>
            );
          })}
        </div>
