...
export function IpnakBallRegistryTab({ initialItems, initialTotal }: Props) {
  ...
  return (
    <div>
        ...
        {items.map(item => (
          <div key={item.id} className="rounded-xl border border-navy-100 bg-[#0d1b2a] p-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-mono font-semibold text-navy-800 flex-1 min-w-0 truncate">{item.ballId}</span>
              <TagUrlCopyButton url={buildTagUrl("ball", item.ballId)} />
      ...
      {/* NFC 태그 프로그래밍 안내 (접이식) */}
      <NfcTagProgrammingGuide kind="ball" />

      {/* 안내 박스 */}
      <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4 text-xs text-blue-700 leading-relaxed">
        <p className="mb-1 font-semibold">볼 ID 등록 가이드</p>
        <p>
          1. NFC 칩에 <strong>URL(URI) 레코드</strong>로{" "}
          <span className="font-mono">{buildTagUrl("ball", ID_SAMPLE.ball)}</span> 형식의 주소를 씁니다.
          (자세한 방법은 위 <strong>NFC 태그 프로그래밍 안내</strong> 참고)
        </p>
