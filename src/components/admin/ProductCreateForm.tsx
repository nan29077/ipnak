...
import {
  Loader2, Plus, X, ChevronLeft, ChevronRight, ImagePlus, Package,
...
export function ProductCreateForm({ shopTagEnabled }: { shopTagEnabled: boolean }) {
  ...
  const [uploading, setUploading] = useState<string | null>(null); // 'thumb' | 'extra-N'
  ...
  function resetForm() {
    ...
    setPrice(""); setShippingFee("0"); setBuyUrl(""); setFeeRate("10");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // extraImages를 options JSON에 함께 저장
  ...
  const modal = open && typeof document !== "undefined" && createPortal(
    <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/70 backdrop-blur-[4px] sm:items-center">
      <div
        className="flex w-full flex-col overflow-hidden rounded-t-[28px] sm:max-w-[520px] sm:rounded-[24px]"
        style={{ background: "linear-gradient(160deg,#0c1e2e 0%,#0d1b2a 100%)", maxHeight: "92dvh" }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* 상단 헤더 */}
        <div className="h-[3px] w-full bg-gradient-to-r from-orange-700/40 via-orange-400 to-orange-700/40" />
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4">
                ...
                <label className="block cursor-pointer">
                  {imageUrl ? (
                    <div className="relative">
                      <img src={imageUrl} alt="" className="h-40 w-full rounded-2xl object-cover ring-2 ring-orange-500/50" />
                      <button
                        type="button"
                    ...
                    <div className={cn(
                      "flex h-40 w-full flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-white/[0.12] bg-white/[0.03] transition-colors hover:border-orange-400/60 hover:bg-orange-500/5",
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-[12px] font-semibold text-white/50">가격 (원)</label>
                <input
                  type="number"
                  className={inputCls}
                  value={price}
              ...
              <div>
                <label className="mb-1.5 block text-[12px] font-semibold text-white/50">구매 링크</label>
                <input
                  className={inputCls}
                  value={buyUrl}
              ...
              <div>
                <label className="mb-1.5 block text-[12px] font-semibold text-white/50">
                  수수료 (%)
                  {!shopTagEnabled && <span className="ml-1.5 text-[10px] font-normal text-white/30">쇼핑 태그 비활성화</span>}
                </label>
                <input
                  type="number"
                  className={cn(inputCls, !shopTagEnabled && "opacity-40")}
                  value={feeRate}
                  onChange={(e) => setFeeRate(e.target.value)}
                  placeholder="10"
                  min={0}
                  max={100}
                  disabled={!shopTagEnabled}
                />
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              {/* 등록 정보 요약 */}
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3 space-y-1.5">
                <p className="text-[11px] font-bold text-white/40 uppercase tracking-wide">등록 정보 확인</p>
                ...
                <div className="flex items-center gap-3 text-[12px] text-white/50">
                  ...
                  {Number(shippingFee) === 0 ? <span className="text-green-400">무료배송</span> : <span>배송비 {Number(shippingFee).toLocaleString()}원</span>}
                </div>
                {imageUrl && (
        ...
        <div className="border-t border-white/[0.06] px-5 py-4">
          <div className="flex gap-2.5">
            ...
            {step < STEPS.length - 1 ? (
              <button
                type="button"
                onClick={() => { if (canNext()) setStep((s) => s + 1); else toast("상품명을 입력해 주세요", "error"); }}
                disabled={!!uploading}
