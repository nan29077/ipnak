...
export default async function AdminTournaments() {
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        {/* 모바일에서 입력하기 편하도록 1열 세로 배치 — 시작일/종료일만 나란히(half) 둔다 */}
        <CreateForm actionType="TOURNAMENT_CREATE" title="대회 생성" singleColumn fields={[
          ...
          { name: "startAt", label: "시작일", type: "date", required: true, half: true },
          { name: "endAt", label: "종료일 (23:59까지)", type: "date", required: true, half: true },
      ...
      <div className="hidden md:block">
        <Table head={["대회명", "타입", "대상어", "상태", "기간", "참가", "참가비 · 보상", "관리"]}>
          ...
          {tournaments.map((t) => {
            ...
            // 배지도 보상 지급 조건과 같은 기준(기간)으로 판단한다 — status 컬럼은 스로틀 때문에 잠시 어긋난다
            const status = effectiveStatus(t);
            const payable = status === "ENDED" && hasReward(rewards) && !t.rewardPaid;
            return (
            <tr key={t.id}>
              ...
              <td className="px-4 py-3"><Badge tone={STATUS_TONE[status] ?? "gray"}>{STATUS_KO[status] ?? status}</Badge></td>
        ...
        {tournaments.map((t) => {
          ...
          <div key={t.id} className="rounded-2xl border border-navy-100 bg-white px-3 py-3 shadow-card">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge tone={STATUS_TONE[status] ?? "gray"}>{STATUS_KO[status] ?? status}</Badge>
            </div>
