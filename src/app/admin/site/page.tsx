...
import { Megaphone, Settings2, Monitor, Sliders } from "lucide-react";
...
const TABS = [
  ...
  { key: "appmode", label: "앱 모드", icon: Sliders },
];
...
export default async function AdminSite({ searchParams }: { searchParams: { tab?: string } }) {
  const tab =
    searchParams.tab === "general"
      ...
      ? "appmode"
      : "banners";
  const [banners, shopEnabled, pcMarginBg, bassOnlyMode, reservationEnabled] = await Promise.all([
