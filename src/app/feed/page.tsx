...
export default async function FeedPage() {
  ...
  const [posts, banners, bassOnlyMode] = await Promise.all([
  ...
  const visiblePosts = bassOnlyMode ? posts.filter(isBassOnlyPost) : posts;
  return <FeedList posts={visiblePosts} currentUserId={user?.id} banners={banners.map((b) => ({ title: b.title, imageUrl: b.imageUrl, linkUrl: b.linkUrl }))} />;
}
