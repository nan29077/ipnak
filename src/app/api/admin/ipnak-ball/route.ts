...
import { ensureIpnakRawColumns } from "@/lib/ipnakProduct";
...
export async function PATCH(req: Request) {
  try {
    ...
    // 재고 복원 판단에 이전 상태·수량이 필요하다. productId는 raw 컬럼이라 따로 읽는다.
    const prevRows = await prisma.$queryRawUnsafe<{ status: string; quantity: number; productId: string | null }[]>(
      `SELECT "status", "quantity", "productId" FROM "BallOrder" WHERE "id" = ?`, b.id
    );
    ...
    await prisma.adminLog.create({ data: {
      ...
      target: b.id,
      detail: restored > 0 ? `${order.orderNo} (재고 +${restored} 복원)` : order.orderNo,
    }});
