...
export type ResolvedSection = {
  ...
  speciesName?: string | null;  // SPECIES 섹션일 때 어종 (배스필터 등 클라이언트 필터링용)
...
export async function getMainSections(limitPerSection = 10): Promise<ResolvedSection[]> {
  ...
  return Promise.all(rows.map(async (r) => {
    ...
    const base: ResolvedSection = {
      key: r.key, title, type: r.type, mode: r.mode,
      style: styleFor(r.type, params, r.key), more, posts,
      speciesName: r.type === "SPECIES" ? (params.species ?? null) : null,
    };
