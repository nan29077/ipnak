...
export async function getProfileData(userId: string, viewerId?: string) {
  ...
  const [postCount, followerCount, followingCount, posts, points, entries, isFollowing] = await Promise.all([
  ...
  const maxSize = Math.max(0, ...posts.map((p) => p.sizeCm ?? 0), ...points.map((p) => p.sizeCm ?? 0));
  ...
  return {
    ...
    stats: { postCount, followerCount, followingCount, topSpecies, maxSize: maxSize || null, pointCount: points.length },
    posts: posts.map((p) => ({ id: p.id, image: p.images[0]?.url ?? null, postType: p.postType, sizeCm: p.sizeCm, speciesName: p.speciesName })),
