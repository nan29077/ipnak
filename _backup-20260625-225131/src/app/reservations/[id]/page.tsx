...
import { Star, MapPin, Users, Fish, Check } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { getCurrentUser } from "@/lib/auth";
import { PageHeader, Card, Badge } from "@/components/ui";
import { BookingForm } from "@/components/BookingForm";
import { reservationCategoryLabel } from "@/lib/taxonomy";
import { safeJson, won } from "@/lib/utils";
...
export default async function ReservationDetailPage({ params }: { params: { id: string } }) {
  ...
  const slots = l.slots.map((s) => ({ date: format(s.date, "yyyy-MM-dd"), timeLabel: s.timeLabel, remaining: s.capacity - s.booked }));
