  ...
  return (
    <div className="pb-28">
      ...
      <div className="space-y-4 p-4">
        ...
        {p.seller && (
          <Link href={`/profile/${p.seller.id}`} className="flex items-center gap-3 rounded-2xl border border-navy-100 bg-white p-3 shadow-card transition-shadow hover:shadow-cardhover">
            <img src={p.seller.avatarUrl || ""} alt="" className="h-10 w-10 rounded-full object-cover ring-1 ring-navy-100" />
      ...
      <div className="pb-safe fixed inset-x-0 bottom-0 z-40 border-t border-navy-100 bg-white/90 p-3 backdrop-blur-md md:relative md:border-0 md:bg-white">
        <div className="mx-auto flex max-w-[640px] gap-2">
          <a href={p.buyUrl && p.buyUrl !== "#" ? p.buyUrl : undefined} target="_blank" rel="noreferrer"
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-navy-700 px-4 py-3 text-[15px] font-semibold text-white shadow-soft btn-press transition-colors outline-none hover:bg-navy-800 focus-visible:ring-2 focus-visible:ring-aqua-300 focus-visible:ring-offset-1">
