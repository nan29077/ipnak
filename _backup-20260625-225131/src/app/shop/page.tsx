    ...
    <div className="pb-24">
      ...
      {products.length === 0 ? (
        <EmptyState
          ...
          action={<LinkButton href="/">피드로 돌아가기</LinkButton>}
        />
      ) : (
        <div className="grid grid-cols-2 gap-3 p-4 md:grid-cols-3">
          {products.map((p) => (
            <Link
              key={p.id}
              href={`/shop/${p.id}`}
              className="group flex flex-col overflow-hidden rounded-2xl border border-navy-100 bg-white shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-cardhover"
            >
