...
const ENV_KEYS = [
  ...
  { key: "COMMERCE_AFFILIATE_KEY", desc: "쇼핑 태그 커머스/제휴 연동" },
];
          ...
          <div className="space-y-1.5">
            ...
            {logs.map((l) => (
              <div key={l.id} className="flex items-center gap-2 text-[12px]">
                <span className="rounded bg-navy-700 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-white">{l.action}</span>
                <span className="truncate text-navy-500">{l.detail || l.target || ""}</span>
                <span className="ml-auto shrink-0 text-navy-300">{format(l.createdAt, "MM.dd HH:mm")}</span>
              </div>
            ))}
          </div>
        </Card>
