  ...
  return (
    <div className="pb-10">
      ...
      <div className="flex flex-col gap-3.5 px-4 py-5">
        ...
        <Card className="space-y-2.5 bg-navy-50/40 p-4">
          <button onClick={() => setShareToFeed((v) => !v)} className="flex w-full items-center justify-between">
            <span className="text-sm font-semibold text-navy-700">피드에도 공유하기</span>
            <span className={`flex h-6 w-11 items-center rounded-full p-0.5 transition ${shareToFeed ? "bg-navy-700" : "bg-navy-200"}`}>
      ...
      <Sheet open={rulerOpen} onClose={() => setRulerOpen(false)} title="스마트 자 · 사이즈 측정">
        {photos[0] && (
          <SmartRuler imageUrl={photos[0].preview} onComplete={(r) => { setRuler(r); setSize(String(r.measuredLengthCm)); setRulerOpen(false); toast(`측정 완료: ${r.measuredLengthCm.toFixed(1)}cm`, "success"); }} />
        )}
      </Sheet>
    </div>
  );
