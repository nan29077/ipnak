...
export function BookingForm({ listingId, price, maxPeople, slots, loggedIn }: {
  ...
  const [date, setDate] = useState(slots[0]?.date ?? format(addDays(new Date(), 1), "yyyy-MM-dd"));
  ...
  return (
    <>
      <div className="pb-safe fixed inset-x-0 bottom-0 z-40 border-t border-navy-100 bg-white p-3 md:relative md:border-0 md:p-0">
            ...
            <Select value={date} onChange={(e) => setDate(e.target.value)}>
              {slots.map((s, i) => (
                <option key={i} value={s.date} disabled={s.remaining <= 0}>
                  {format(new Date(s.date), "M월 d일")} {s.timeLabel} · {s.remaining > 0 ? `잔여 ${s.remaining}` : "마감"}
                </option>
              ))}
            </Select>
          </div>
