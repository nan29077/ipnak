  ...
  return (
    <div>
      ...
      <div
        ...
        onWheel={onWheel}
        className="relative w-full select-none overflow-hidden rounded-xl bg-navy-900"
        ...
        <div className="flex gap-1">
          {[10, 30, 50].map((v) => (
            <button key={v} onClick={() => setCalCm(v)}
              className={`rounded-lg px-3 py-1.5 text-sm font-medium ${calCm === v ? "bg-navy-700 text-white" : "bg-navy-50 text-navy-600"}`}>{v}cm</button>
          ))}
        </div>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button onClick={complete} disabled={measuredCm == null}
          className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-navy-700 py-3 text-sm font-semibold text-white disabled:opacity-40">
          <Check size={16} /> 측정 완료
        </button>
      </div>
    </div>
...
function ToolBtn({ onClick, label, children, disabled }: { onClick: () => void; label: string; children: React.ReactNode; disabled?: boolean }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="flex items-center gap-1 rounded-lg border border-navy-100 bg-white px-2.5 py-1.5 text-xs font-medium text-navy-700 disabled:opacity-40"
    >
      {childr