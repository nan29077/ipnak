...
function FeedCardImpl({ post, currentUserId }: { post: FeedPost; currentUserId?: string }) {
  ...
  return (
    <article className="border-b border-navy-100 bg-white md:mb-3 md:rounded-2xl md:border md:border-navy-100 md:shadow-card">
      {/* 헤더 */}
      ...
      <div className="relative aspect-square w-full select-none bg-navy-50">
        ...
        {showTags && post.productTags.map((t) => (
          <Link key={t.id} href={t.product.buyUrl && t.product.buyUrl !== "#" ? t.product.buyUrl : `/shop/${t.product.id}`}
            className="absolute z-10 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full bg-white/95 px-2 py-1 text-[11px] font-semibold text-navy-800 shadow"
        ...
        {/* 사이즈 뱃지 */}
        {post.sizeCm != null && (
          <span className="absolute left-2.5 top-2.5 inline-flex items-center gap-1 rounded-full bg-navy-900/80 px-2 py-0.5 text-[11px] font-semibold text-white backdrop-blur">
        ...
        {post.productTags.length > 0 && (
          <button onClick={() => setShowTags((v) => !v)} aria-label="쇼핑 태그 보기"
            className="badge absolute bottom-2.5 right-2.5 bg-white/90 text-navy-700 shadow-soft backdrop-blur-sm btn-press">
        ...
        {multi && (
          <>
            ...
            <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-1">
              {post.images.map((_, i) => <span key={i} className={cn("h-1.5 w-1.5 rounded-full", i === idx ? "bg-white" : "bg-white/50")} />)}
            </div>
          </>
        )}
      </div>
    ...
    <Sheet open={open} onClose={() => { onClose(); setLoaded(false); }} title="댓글">
      ...
      <div className="mt-2 flex items-center gap-2 border-t border-navy-100 pt-3">
        <input
          ...
          className="flex-1 rounded-full border border-navy-100 bg-navy-50 px-4 py-2.5 text-sm text-navy-800 placeholder-navy-300 outline-none transition focus:border-aqua-400 focus:bg-white focus:ring-2 focus:ring-aqua-100"
        />
        <button onClick={send} aria-label="댓글 전송" className="rounded-full bg-navy-700 p-2.5 text-white shadow-soft btn-press transition-colors hover:bg-navy-800"><Send size={16} /></button>
      </div>
    </Sheet>
  );
}
...
const CommentRow = memo(function CommentRow({ c }: { c: any }) {
  return (
    <div className="flex items-start gap-2">
      <img src={c.author.avatarUrl || "https://i.pravatar.cc/60"} alt="" loading="lazy" decoding="async" className=