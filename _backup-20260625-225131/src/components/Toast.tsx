    ...
    <ToastCtx.Provider value={push}>
      ...
      <div className="fixed left-1/2 bottom-24 z-[1000] flex w-[calc(100vw-2rem)] max-w-sm -translate-x-1/2 flex-col gap-2 md:bottom-8">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="animate-toast flex items-center gap-2.5 rounded-2xl border border-white/10 bg-navy-900 px-3.5 py-3 text-sm font-medium text-white shadow-card"
            role="status"
          >
            <span
              className={
